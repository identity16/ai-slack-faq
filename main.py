...
def generate_slack_faq(channel_name: str, days: int, output_file: str = None) -> str:
    """
    슬랙 스레드에서 FAQ 생성
...
def generate_ut_debrief(notion_doc_id: str, output_file: str = None) -> str:
    """
    ...
    print(f"\nUT Debrief가 '{output_file}' 파일에 저장되었습니다.")
    return output_file
...
def main():
    ...
    if args.command == "faq":
        generate_slack_faq(args.channel, args.days, args.output)
    elif args.command == "ut":
        generate_ut_debrief(args.doc_id, args.output)
    else:
        parser.print_help()
